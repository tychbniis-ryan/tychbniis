...
## 最近一次修改摘要：0.64.0 - 2026-07-03

1. GAS 新增設站資料送出檢核改為使用彈出式全域訊息，不再把錯誤清單顯示在頁面最上方。
2. 「送出並公告」補強為所有主要填報欄位均需填寫，包含是否申請宣導品、備註、填報單位與填報人。
3. 本機檢查新增送出檢核與頁首錯誤區隱藏驗收，避免後續回退成頁首錯誤清單。
4. 0.63.0 的接種回報時間、分類明細欄位與 3 天內修改判斷仍保留。
5. 0.62.0 的民眾端 LINE 使用提示關閉、GAS 手機首頁單欄大按鈕、回報清冊與彈出視窗仍保留。
6. 0.61.0 的民眾端快速查詢、卡片按鈕、廠牌篩選、GAS 新增設站、維護篩選、查詢場次、廠商配送與宣導品管理密碼調整仍保留。
7. 本機已通過 `npm run test:local` 與 GAS 語法檢查。
