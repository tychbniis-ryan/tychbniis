...
## 最近一次修改摘要：0.65.0 - 2026-07-03

1. GAS 產生 `public.json` 時新增公開的 `villages` 參照欄位，只輸出啟用中的行政區與里別。
2. 民眾端里別關鍵字候選優先使用 `public.json` 的 `villages`，依所選行政區篩選；舊版 JSON 沒有此欄位時才退回從場次資料推算。
3. `PUBLIC_JSON_SPEC.md` 與本機檢查同步允許並驗證 `villages` 欄位，避免輸出里別清冊內部狀態、備註與最後更新時間。
4. 0.64.0 的新增設站資料彈出式送出檢核與完整公告欄位檢核仍保留。
5. 0.63.0 的接種回報時間、分類明細欄位與 3 天內修改判斷仍保留。
6. 0.62.0 的民眾端 LINE 使用提示關閉、GAS 手機首頁單欄大按鈕、回報清冊與彈出視窗仍保留。
7. 0.61.0 的民眾端快速查詢、卡片按鈕、廠牌篩選、GAS 新增設站、維護篩選、查詢場次、廠商配送與宣導品管理密碼調整仍保留。
8. 本機已通過 `npm run test:local` 與 GAS 語法檢查。
