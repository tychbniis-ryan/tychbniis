...
## 最近一次修改摘要：0.61.0 - 2026-07-03

1. 民眾端手機首屏表頭縮小，快速查詢只保留「查詢今日場次」與「查附近接種站」，其他快捷移入「進階查詢」。
2. 民眾端里別改為關鍵字輸入搭配 datalist；選擇流感或新冠疫苗後，會顯示可勾選的廠牌篩選。
3. 民眾端查詢結果區新增「調整查詢」按鈕，方便使用者跳回篩選條件重新查詢。
4. 民眾端接種站卡片上方提供小圖示「複製場次資訊」與「分享 Google Maps 位置」，地址旁提供複製地址，底部只保留「開啟導航」與「查看現場資訊」。
5. 民眾端 `public/public.json` 已移除範例場次，避免正式畫面誤用假資料；正式資料仍需由 GAS `buildPublicJson()` 產生後放入。
6. GAS 新增設站移除必填紅點與暫存轉草稿說明文字，里別改為「選 1 個里後加入」，接種日期不得早於今天，設站時間預設上午 08:00 至 12:00。
7. GAS 新增設站按鈕改為「送出並公告」、「上傳不公告」、「暫存不送出」；暫存不受必填限制，送出並公告會做完整欄位檢核。
8. GAS 維護既有資料新增行政區、狀態與關鍵字篩選；查詢場次不再顯示維護用功能鈕。
9. GAS 回報接種人數新增當日、非當日與修改入口，前端分類輸入後加總寫回既有總表欄位，分類明細寫入接種回報備註。
10. 廠商配送回報改為輸入「廠商管理密碼」，不再使用廠商查詢碼；後端優先讀系統設定「廠商管理密碼」，未填時暫用「管理功能密碼」。
11. 宣導品管理入口可使用管理功能密碼或廠商管理密碼通過驗證，其他管理頁仍維持管理功能密碼。
12. `scripts/local-check.mjs` 與 `scripts/rwd-check.mjs` 已同步新版驗收；本機已通過 `npm run test:local` 與 `node scripts/rwd-check.mjs`。
