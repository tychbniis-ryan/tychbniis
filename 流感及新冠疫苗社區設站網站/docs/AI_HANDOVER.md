...
## 最近一次修改摘要：0.63.0 - 2026-07-03

1. 設站資料表新增 `接種回報時間` 與 `接種回報明細` 欄位，讓「填報日 3 天內修改」與分類明細保存有明確資料依據。
2. `updateReport()` 會將分類加總寫回流感／新冠總人數，將分類明細寫入 `接種回報明細`，將使用者輸入備註保留在 `接種回報備註`。
3. `修改接種人數` 改依 `接種回報時間` 判斷是否在 3 天內，不再用 `最後更新時間` 代替。
4. `ensureSheet_()` 會對既有工作表追加缺少欄位，不刪除、不搬移既有欄位與資料。
5. 維護／查詢詳細資料列會顯示接種回報時間、明細與備註，方便後續追蹤。
