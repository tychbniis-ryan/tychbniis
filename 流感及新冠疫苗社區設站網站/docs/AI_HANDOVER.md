...
## 最近一次修改摘要：0.68.0 - 2026-07-03

1. GAS 首頁功能入口已統一為「回報作業」、「設站作業」、「管理作業」三大區塊；靜態 HTML 與 `configureHomeNavigation()` 動態產生結果一致。
2. 新增設站頁不再顯示送出前預覽，`資料狀態` 改為隱藏欄位，送出時由「上傳不公告」或「送出並公告」按鈕決定。
3. 新增設站頁的必填欄位改以 `required-field` 紅點標示，不再於欄位名稱重複顯示「必填」文字。
4. 配送任務填寫表單移除「實際配送日期」輸入；後端仍保留既有欄位相容，避免破壞舊資料表。
5. `scripts/local-check.mjs` 已補上上述 UI 與流程驗收，避免後續回退。
...
2. 線上檢查會驗證每筆 `villages` 只包含 `district` 與 `village`，避免公開 JSON 混入不必要欄位。
3. 本次屬測試腳本修正，網站資料格式與 GAS 功能未變更。

## 最近一次修改摘要：0.67.0 - 2026-07-03

1. GAS 廠商端登入已改為只使用系統設定表的「廠商管理密碼」，不得再用「管理功能密碼」代替。
2. 「廠商資料表」會透過 `buildReferenceData_()` 提供到 GAS 前端，配送表單與配送編輯表單可使用廠商下拉選單。
3. 選擇廠商後，前端會自動帶出「廠商聯絡人」與「廠商聯絡電話」，欄位為唯讀，避免人工誤填。
4. 廠商端配送任務改成 `record-row` 可展開清冊，預設只顯示任務、地點、狀態與回報按鈕，明細表格化呈現。
5. 本次已通過 `npm run test:local`。若部署後廠商端無法登入，請先確認 Google Sheet「系統設定表」已填入「廠商管理密碼」。

## 最近一次修改摘要：0.66.0 - 2026-07-03

1. GAS 回報接種人數清冊再精簡，只在清冊列顯示接種日期與接種場地名稱，回報狀態與行政區／里別不再顯示於清冊列。
2. 回報、修改按鈕仍保留在清冊列，點擊後才開啟分類回報彈出視窗。
3. 本機檢查新增回報清冊精簡驗收，避免後續回退成資訊過多的清冊列。
4. 0.65.0 的 `public.json.villages` 公開里別參照與民眾端里別候選來源調整仍保留。
5. 0.64.0 的新增設站資料彈出式送出檢核與完整公告欄位檢核仍保留。
6. 0.63.0 的接種回報時間、分類明細欄位與 3 天內修改判斷仍保留。
7. 0.62.0 的民眾端 LINE 使用提示關閉、GAS 手機首頁單欄大按鈕、回報清冊與彈出視窗仍保留。
8. 0.61.0 的民眾端快速查詢、卡片按鈕、廠牌篩選、GAS 新增設站、維護篩選、查詢場次、廠商配送與宣導品管理密碼調整仍保留。
9. 本機已通過 `npm run test:local` 與 GAS 語法檢查。
...
1. 讀取 `public.json`。
2. 顯示網站標題、公告、更新時間。
3. 快速查詢預設只顯示今日場次與附近接種站；明日、本週與全部場次放在進階查詢。
...
30. 進入稽催／統計、宣導品管理、系統工具前，需先通過管理碼驗證；廠商配送回報使用「廠商管理密碼」，不再使用廠商查詢碼。
31. 稽催／統計頁會顯示宣導品配送完成率與行政區設站統計。
32. `callGas()` 執行期間會停用畫面按鈕，避免重複點擊造成重複送出。
33. 表單錯誤訊息會透過彈出式全域訊息顯示問題清單與「下一步」操作提示，不再顯示在頁面最上方。
...
3. 依使用者選擇的條件篩選資料；里別 datalist 優先由 `public.json.villages` 產生，沒有該欄位時才由場次資料推算。
4. 將接種站資料渲染為卡片。
...
36. 表單錯誤會透過 `formatErrorMessage()` 與彈出式全域訊息顯示「請先修正下列問題」與下一步。
...
46. `scripts/local-check.mjs` 會檢查 `public/public.json` 只含公開欄位，並阻擋醫療院所代碼、接種人數、接種率、填報人、管理碼、宣導品配送聯絡資料與里別清冊內部欄位等資訊。
