...
0.6.7 補充：GAS 預設題庫改為 `臺灣生活趣味問答.md` 的 20 題，移除自動補回 `demo_q001` 至 `demo_q011` 測試題；新增講師端「匯入臺灣題庫」按鈕與管理密碼保護的 `replaceQuestionBankWithTaiwanQuestions` 題庫覆寫 action。關題計分改為只同步本題需要的道具，並加入玩家同步短暫快取，降低每次關題重複讀 Firebase 與全域道具同步的時間。
