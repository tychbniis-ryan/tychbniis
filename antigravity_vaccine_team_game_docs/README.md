...
第 4 版已於 2026-05-23 以 `0.4.0-planning` 啟動。

第 4 版定位為「靜態 HTML5 優先、低 GAS 呼叫、免費方案穩定版」，先修正路線圖與交接紀錄，不直接修改第 3 版已定版的學員端、講師端、GAS 或 Firebase 部署設定。
...
目前 `0.4.27` 仍為第 4 版測試修正版，尚未定版。學員端只顯示個人積分；投影端開題後顯示 65 秒倒數；GAS 結算前會同步最後關題後送出的道具紀錄，並過濾全對獎候選紀錄，避免結算名單重複顯示。
