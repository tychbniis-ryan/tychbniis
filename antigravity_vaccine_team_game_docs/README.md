0.7.28 treasure state preserve：2026-06-10 修正講師端直接用 Firebase 發送追加寶箱或落後寶箱後，GAS 開題、關題、背景計分或最終結算整包寫回 `gameState` 時，可能用試算表舊狀態覆蓋 Firebase 最新寶箱欄位的問題。GAS 現在會在發布 `gameState` 前合併 Firebase 最新 `additionalTreasureBox*` 與 `laggingTreasureBox*` 欄位。已檢查目前已結算資料：GAS 與 Firebase 排行榜一致，`team_1 = 答題 120 + 道具 89 = 總分 209`，AAA `174`，Bbb `155`。本次不改答題與道具計分公式，學員端 `clientVersion` 保持不變，只更新靜態檔快取參數。Firebase Hosting 已部署，GAS Web App deployment 為 @114。
...
0.7.22 comeback card immediate apply：2026-06-09 修正學員端翻身卡按下使用後未立即刷新分數的問題。翻身卡送出 Firebase 成功後，前端會立即標記為已套用、刷新本地背包與個人分數；分數規則維持第 6 版定版邏輯，未改道具卡計分規則。
# 第 7 版作業：關題效能量測
...
2. 成就完成後，回答頁提示會直接重新計算本機成就狀態，不需先點開成就面板才出現提醒。
3. 道具使用後會同步更新成就提示，特別是「使用 3 張道具」這類本機成就。
4. 修正加分卡使用後的畫面文字，避免直接加分卡誤顯示為「下一題套用」。
5. 已確認第 6 版定版道具規則並保留不改：
   - 加分卡 `+1/+3/+5/+10`：當下計入道具分。
   - 挑戰卡：猜大小後當下計入道具分。
   - 翻身卡：依目前關題的 `comebackControl` 當下計入道具分。
   - 加倍卡：唯一下一題套用，下一題答對時把該題答題分加倍。
6. 本次不改 Firebase rules、不改 GAS 計分公式、不改寶箱或道具資料結構；僅更新學員端提示同步與快取版本。

## 0.7.19 回復第 6 版寶箱流程與清除舊快照

本版處理實機回報：

1. 寶箱與翻身卡流程偏離第 6 版設計。
2. 重新啟用後，學員端或投影端仍可能看到上一題或上一場排行榜快照。

修正方式：

1. 學員端移除上一版新增的 Firebase `EventSource` 即時推播監聽，回到第 6 版輪詢流程。
2. 翻身卡等待本題結算時，回復第 6 版 15 秒後自動再確認。
3. 保留第 7 版 Firebase 作為主要資料來源，但不新增新的寶箱或翻身卡操作流程。
4. 等待開題時，學員端會清空上一題畫面。
5. 投影端與學員端排行榜會略過早於目前場次啟用時間的舊快照。

還原方式：回退本次 Git commit，或回復 `frontend/student/dist/app.js`、`frontend/instructor/dist/display.js` 與版本參數後重新部署 Firebase Hosting。

---

## 0.7.18 即時寶箱、投影排行榜與翻身卡修正

本版修正 4 個實機問題：

1. 清空資料後，講師端寶箱按鈕沒有立即重置。
2. 投影端排行榜仍可能讀舊設定或用舊排序欄位。
3. 講師發送追加寶箱或落後寶箱後，學員端需要重新整理才看到寶箱。
4. 翻身卡在結算控制資料尚未抵達時，容易顯示無法使用或沒有立即套用。

修正方式：

1. 清空資料與啟動場次後，講師端會立即依最新狀態重繪寶箱按鈕。
2. 投影端 `Display.html` 改載入第 7 版設定 `config-v7-test.js`，並更新快取參數。
3. 投影端排行榜優先使用 `weightedAverageScore` 排序與顯示。
4. 學員端收到寶箱狀態後，會立即重繪本機寶箱與道具清單。
5. 翻身卡等待 `comebackControl` 時，會縮短為 3 秒後重新讀取狀態並重試。

還原方式：回退本次 Git commit，或回復上述前端檔案後重新部署 Firebase Hosting；若 GAS 也已部署，回復 `gas/Code.gs` 後重新部署 GAS。
