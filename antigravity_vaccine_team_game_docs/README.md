...
第 3 版已完成雲端部署，目前版本標記為 `0.3.8-deployed`。
...
目前第 3 版已完成寶箱資料表、關題後寶箱取得判定、每人最多 3 個未開啟寶箱限制、開寶箱 API、道具庫讀取、基本道具效果、幸運獎與全對獎結算、戰隊加權平均分排行榜、學員端寶箱與道具 UI、創作題投稿、隊內初選、講師審核代表作品、匿名全體投票與賽後報表匯出，並已部署 GAS Web App 與 Firebase Hosting。
...
3. 先完成本機測試，再由使用者確認是否推送雲端。
4. 未經確認不得執行 `firebase deploy`、`clasp push` 或 `clasp deploy`。
5. 正式活動前可在講師端按「初始化遊戲資料」，清空測試玩家、作答、翻卷與排行榜資料；題庫與戰隊設定會保留。
6. 預設測試題目前為 3 題：`demo_q001`、`demo_q002`、`demo_q003`。
