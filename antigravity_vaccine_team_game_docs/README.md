...
第 5 版已於 2026-05-28 推進至 `0.5.11` UI 文字、寶箱流程與投影端結算修正版。

第 5 版定位為「Pixel Art Retro Game UI 教學展示版」。`0.5.11` 主要精簡學員端提示文字、修正寶箱與道具紀錄流程、改善投影端結算畫面，讓現場操作畫面更像遊戲介面而不是系統後台。
...
| 快照 | `screenshots/v5_0_5_1/` 至 `screenshots/v5_0_5_11/` |
| 後端狀態 | `0.5.8` 修正翻身卡唯一最後 1 名判定，GAS Web App deployment `@52` |

`0.5.1` 至 `0.5.11` 已完成寶箱、道具、答題回饋、講師端控制流程、投影端排行榜、結算畫面、戰隊識別、挑戰卡抽號碼、成就圖示、PixelArt Icons 套用、獎盃圖資、狀態顯示簡化、UI 文字精簡與手機版 RWD 視覺補強。
