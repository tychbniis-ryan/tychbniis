...
第 5 版已於 2026-05-28 推進至 `0.5.15` 講師端確認視窗修正版。

第 5 版定位為「Pixel Art Retro Game UI 教學展示版」。`0.5.15` 主要把講師端結算、清空測試資料與重新開題的瀏覽器原生確認視窗，改為遊戲內像素風確認視窗。
...
| 後端狀態 | `0.5.15` 未修改 GAS，沿用 GAS Web App deployment `@53` |

`0.5.1` 至 `0.5.15` 已完成寶箱、道具、答題回饋、講師端控制流程、投影端排行榜、結算畫面、戰隊識別、挑戰卡抽號碼、成就圖示、PixelArt Icons 套用、獎盃圖資、狀態顯示簡化、UI 文字精簡、遊戲內確認視窗與手機版 RWD 視覺補強。
