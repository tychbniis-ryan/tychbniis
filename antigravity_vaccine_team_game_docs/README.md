...
第 5 版已於 2026-05-28 推進至 `0.5.10` 狀態顯示簡化修正版。

第 5 版定位為「Pixel Art Retro Game UI 教學展示版」。`0.5.10` 主要移除循環箭頭與綠橘條紋等待動畫，狀態區改為純文字顯示，避免畫面出現不必要的干擾圖示。
...
`0.5.1` 至 `0.5.10` 已完成寶箱、道具、答題回饋、講師端控制流程、投影端排行榜、結算畫面、戰隊識別、挑戰卡抽號碼、成就圖示、PixelArt Icons 套用、獎盃圖資、狀態顯示簡化與手機版 RWD 視覺補強。
