...
第 5 版已於 2026-05-28 推進至 `0.5.14` 講師端初始化、挑戰卡揭曉與標籤置中修正版。

第 5 版定位為「Pixel Art Retro Game UI 教學展示版」。`0.5.14` 主要修正講師端重新載入後的初始化流程、講師端標題標號、成就狀態置中與挑戰卡抽號揭曉節奏。
...
| 後端狀態 | `0.5.14` 已同步 GAS Web App deployment `@53` |

`0.5.1` 至 `0.5.14` 已完成寶箱、道具、答題回饋、講師端控制流程、投影端排行榜、結算畫面、戰隊識別、挑戰卡抽號碼、成就圖示、PixelArt Icons 套用、獎盃圖資、狀態顯示簡化、UI 文字精簡與手機版 RWD 視覺補強。
