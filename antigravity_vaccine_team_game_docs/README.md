...
目前第 3 版已完成寶箱資料表、關題後寶箱取得判定、每人最多 3 個未開啟寶箱限制、開寶箱 API、道具庫讀取、基本道具效果、幸運獎與全對獎結算、戰隊加權平均分排行榜、學員端浮動寶箱與成就 UI、創作題投稿、隊內初選、講師審核代表作品、匿名全體投票與賽後報表匯出。`0.3.15` 已啟動免費方案效能重構第一階段：學員報到優先寫 Firebase `players`、送答優先寫 Firebase `answers`、道具使用寫 `itemUses` pending、排行榜優先讀 Firebase 快照、寶箱與成就領取改為快速請求，不再於按鈕點擊時等待 GAS / Google Sheets 即時計分或全量刷新。

`0.3.15` 仍保留 GAS / Google Sheets 作賽前同步與賽後正式重算。Cloud Functions、Cloud Run、Blaze 方案均未啟用。
