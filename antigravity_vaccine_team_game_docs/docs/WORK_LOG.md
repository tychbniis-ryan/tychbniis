...
2. 加倍卡在最後 1 題後使用時，學員端紀錄顯示 0 分與無加分效果。
3. 公開獎項與投影端結算名單過濾 `perfect_candidate`，避免全對獎同一位學員重複顯示。
4. 空寶箱提示改為明確說明沒有取得道具、不扣分、不需再操作。
5. 已部署 GAS Web App deployment `@50` 與 Firebase Hosting，線上頁面載入 `0.4.27`。
6. 已完成線上頁面載入檢查與現有 finalized 場次分數讀取；完整重新開局活動測試需先確認是否可清空目前場次。
...
# 2026-05-26?0.4.22 ????
1. ??? GAS?Apps Script Web App deployment ??? `@45`?
2. ??? Firebase Hosting????????????????????? `0.4.22`?

# 2026-05-26?0.4.22 ???????????
1. ????????????????????????????
2. ?????????? 1 ??????????????????????????????
3. ?? `gameSessionSeed`??????????????????? seed?
4. ????????? `gameSessionSeed + playerId + questionId`?????????????????????
...
3. 投影端自動更新不再輪詢 GAS；只有講師或協助者按下手動刷新時，才允許 GAS 補救讀取。
4. 學員端翻開試卷後，若收到時間較舊的 `question_closed`，會忽略該狀態，避免剛開題就被舊關題覆蓋。
