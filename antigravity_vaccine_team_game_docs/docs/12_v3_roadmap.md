...
## 2026-05-23：0.3.20 創作題資料隔離與送答備援

本次完成範圍：

1. 一般題送答遇 Firebase `HTTP 401` 時，前端自動回退 GAS `submitAnswer`。
2. 創作投稿與投票新增 `questionId`，避免舊創作題或測試資料混入目前題目。
3. 講師結算競賽不再同步整場 Firebase `answers`，降低現場結算等待時間。
4. 個人摘要讀取失敗時，學員端會清除寶箱與成就紅點。

## 2026-05-23：0.3.18 快速寫入與 GAS 結算相容修正

本次完成範圍：

1. 關題計分前同步 Firebase `players` 與當題 `answers`，避免講師關題時找不到快速送答資料。
2. 創作題讀取投稿池、隊內投票、講師候選、決選作品、全體投票與結算前同步 Firebase 創作暫存資料。
3. `findPlayer` 找不到 Google Sheets 玩家時，從 Firebase `players` 匯入，避免成績結算、寶箱、成就讀取失敗。
4. 寶箱與成就讀取失敗時先隱藏紅點，避免沒有可操作項目仍顯示警示。
