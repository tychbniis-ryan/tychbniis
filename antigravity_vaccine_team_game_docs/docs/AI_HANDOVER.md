# 最近一次修改摘要：0.4.25 分數口徑與重新開題修正
1. 學員端頂端狀態列已改為「個人積分(答題/道具)」，例如 `200分（180/20）`。
2. 個人積分統一為答題得分加道具加分；個人排行榜依個人積分排序，並保留總作答秒數。
3. 戰隊積分統一為每題參與者答題平均分加總，再加全隊道具加分。
4. 排行榜已移除答對率顯示，因挑戰卡已改為前端猜大小即時計分。
5. 連續答對成就已正規化 `true` / `false` 判斷，避免答錯後進度未重新計算。
6. 學員端作答時間改為 65 秒；剩餘秒數大於 60 秒時，送出秒數固定記為 1 秒。
7. 講師端新增「重新開題」按鈕，供偶發自動關題或未作答學員補答；已作答學員仍不能重複送出。
8. 版本已更新為 `0.4.25`，模組狀態為 `v4_0_4_25_scoreboard_reopen_fix`。
9. GAS 已推送並部署到 Apps Script Web App deployment `@48`。
10. Firebase Hosting 已部署，學員端、講師端與投影端線上頁面皆載入 `0.4.25`。
11. 已完成本機與線上 Playwright 載入檢查；線上 GAS `getGameState` 回應 `ok:true`，`reopenQuestion` 未帶管理密碼時回覆授權失敗。
