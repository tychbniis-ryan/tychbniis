# 最近一次修改摘要：0.4.24 學員自動倒數與舊題背景計分保護
1. 第 4 版目前仍為測試修正版，尚未定版。
2. 講師開題後，學員端會自動載入目前題目，並以 Firebase / GAS 提供的 `questionOpenedAt` 作為倒數起點。
3. 學員端「開始作答」只負責打開作答視窗，不再決定計時起點；重複點擊不會重設倒數或作答秒數。
4. 投影端維持開題後立即顯示題目與選項，不做 10 秒延遲。公平性由學員端統一開題時間控制。
5. GAS `scoreClosedQuestionNow()` 加入狀態保護：舊題背景計分完成時，若講師已開新題，只更新排行榜，不覆蓋 Firebase `gameState` 為舊題 `question_closed`。
6. 學員端連續答對成就改為各成就獨立鎖定，`streak_3` 完成後不會阻止 `streak_5` 繼續累積。
7. 學員端關題後本機分數改為 `基本分 + 道具分`，頂端分數會取本機分數與排行榜快照後端分數的較高值，降低顯示落差。
8. 版本已更新為 `0.4.24`，模組狀態為 `v4_0_4_24_fair_answer_flow_fix`。
9. GAS 已推送並部署到 Apps Script Web App deployment `@47`。
10. Firebase Hosting 已部署，學員端、講師端與投影端線上頁面皆回應 `200` 並載入 `0.4.24`。
