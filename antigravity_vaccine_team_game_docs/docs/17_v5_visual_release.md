...
1. 個人排名獎盃改用全新檔名 `award-player-rank-clean-1.png` 至 `award-player-rank-clean-5.png`。
2. 獎盃重新製作為透明背景像素 PNG：第 1 名彩虹、第 2 名紫、第 3 名金、第 4 名銀、第 5 名銅，中央數字放大。
3. 學員端與投影端排行榜改讀新檔名，避免瀏覽器沿用同檔名舊圖。
4. 講師端追加寶箱按鈕會依已啟用箱號改為狀態色並顯示「已開 N」，讓講師能辨識哪一箱已開過。
5. 若講師直接啟用第 N 箱，GAS 與學員端皆會自動視為第 1 箱至第 N 箱已啟用。
6. 落後寶箱採固定種子 30% 發放率，講師啟用指定戰隊後，由該戰隊學員端自行判定是否取得寶箱。
7. 舊獎盃檔案保留但不再作為目前排行榜主要引用來源，以降低還原與相容風險。
8. 依 Web Interface Guidelines 檢查本次 UI，避免只靠顏色表示狀態，並放大學生端個人排行獎盃欄位。
