...
2. 學員端與投影端排行榜改讀新檔名，避免瀏覽器沿用同檔名舊圖。
3. 講師端追加寶箱按鈕會依已啟用箱號改為綠色，讓講師能辨識哪一箱已開過。
4. 落後寶箱採固定種子 30% 發放率，講師啟用指定戰隊後，由該戰隊學員端自行判定是否取得寶箱。
5. 舊獎盃檔案保留但不再作為目前排行榜主要引用來源，以降低還原與相容風險。
6. 本版只需部署 Firebase Hosting，GAS Web App deployment 維持 `@57`。
