...
1. 投影端開題與等待畫面的戰隊排行榜改為較緊湊的即時排行列，避免列表底部被裁切。
2. 學員端等待開題時只保留小字提示，移除中間重複的大字提示。
3. 關題關題後，把「第 N 題已關題」移到下方對話框，與已選擇答案、花費秒數合併呈現。
4. 空寶箱開啟後顯示大型醒目提示，讓學員知道這次沒有取得道具。
5. 獎勵提示改為指向上方寶箱與成就按鈕，符合手機版操作位置。
6. 成就狀態標籤置中；答題彈窗內只顯示題目，不重複顯示題號。
7. 學員端排行榜彈窗改為像素風手機版清單，並移除快照更新說明文字。
8. 本版截圖位於 `screenshots/v5_0_5_12/`。
