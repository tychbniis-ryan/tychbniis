...
function updateVaccineQuestionBankFromMenu() {
  return updateQuestionBankFromBundledRows(getVaccineQuestionRows().concat(getChildProtectionQuestionRows()), {
    source: 'Vaccine education training question bank markdown bundled in GAS',
    message: 'Vaccine question bank update completed, including child protection and neglect questions. Old vac_q rows not in the new source were disabled. No rows were deleted.',
    disableDemoRows: false,
...
function openQuestion(data, payload) {
  ...
  const state = Object.assign(
    withLatestFirebaseTreasureState(gameId, baseState),
...
function reopenQuestion(data, payload) {
  ...
  const openedAt = new Date().toISOString();
  const nextOpenedQuestionIds = formatOpenedQuestionIds(openedQuestionIds.concat(questionId));
  const state = Object.assign(withLatestFirebaseTreasureState(gameId, {
    gameId,
    status: 'question_open',
    currentQuestionId: questionId,
    questionOpenedAt: openedAt,
...
function pickLatestTimestampValue(firstValue, secondValue) {
...
function mergeTreasureStateFields(baseState, firebaseState) {
  const base = baseState || {};
  const latest = firebaseState || {};
  const mergedAdditionalSlots = mergeCsvListValues(
    base.additionalTreasureBoxSlots,
    latest.additionalTreasureBoxSlots
  ).join(',');
  const mergedLaggingTeams = mergeCsvListValues(
    base.laggingTreasureBoxTeams,
    latest.laggingTreasureBoxTeams
  ).join(',');

  return Object.assign({}, base, {
    additionalTreasureBoxLevel: Math.max(
      0,
      Number(base.additionalTreasureBoxLevel || 0),
      Number(latest.additionalTreasureBoxLevel || 0)
    ),
    additionalTreasureBoxUpdatedAt: pickLatestTimestampValue(
      base.additionalTreasureBoxUpdatedAt,
      latest.additionalTreasureBoxUpdatedAt
    ),
    additionalTreasureBoxSlots: mergedAdditionalSlots,
    laggingTreasureBoxTeams: mergedLaggingTeams,
    laggingTreasureBoxUpdatedAt: pickLatestTimestampValue(
      base.laggingTreasureBoxUpdatedAt,
      latest.laggingTreasureBoxUpdatedAt
    )
  });
}

function withLatestFirebaseTreasureState(gameId, state) {
  const firebaseState = getFirebaseJson('gameState/' + encodeURIComponent(gameId)) || {};
  return mergeTreasureStateFields(state, firebaseState);
}
...
function finalizeCompetition(data, payload) {
  ...
  const currentState = getGameState({ gameId });
  const state = withLatestFirebaseTreasureState(gameId, {
    ...currentState,
    gameId,
    status: 'finalized',
    currentQuestionId: '',
    questionOpenedAt: '',
    sessionStartedAt: currentState.sessionStartedAt || finalizedAt,
    gameSessionSeed: currentState.gameSessionSeed || createGameSessionSeed(gameId, finalizedAt),
...
function getChildProtectionQuestionRows() {
  return [
  [
    "vac_q051",
    51,
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "預防接種門診是協助發覺潛在兒少疏忽或虐待的重要時機。關於醫事人員在預防接種看診時的評估重點，下列敘述何者正確？",
    "若發現幼兒有延遲接種常規疫苗的情形，應直接通報兒少保護，不需再向家長詢問原因。",
    "在兒童未產生任何外傷前，幼兒是否「定期接受預防接種」，是有助於發現潛在兒少「疏忽」個案的好方法。",
    "預防接種門診僅需核對疫苗品項與施打部位，身體其他部位的舊傷痕因與本次看診無關，不須理會。",
    "若幼兒都有如期接種疫苗，即可完全排除其家庭為高風險家庭或有兒虐的可能性。",
    "",
    "B",
    "A： 錯誤。發現預防接種紀錄不完整時，首要步驟是詢問其原因，例如幼兒近期是否反覆生病不宜施打，而非不分青紅皂白直接通報。\nC： 錯誤。預防接種時通常會解開幼兒衣物，正是檢視是否有不合理創傷、嚴重營養不良或未處理皮膚病的好時機。\nD： 錯誤。雖然有按時接種代表家長對醫療資源有一定程度的重視，但身體虐待、精神虐待或性虐待仍可能發生，無法單憑打疫苗就完全排除風險。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "在預防接種門診中，醫師除了為嬰幼兒施打疫苗，也可透過查閱「兒童健康手冊」及身體檢查來評估兒少受虐風險。下列哪些指標或發現，提示醫事人員應進一步評估或通報？\n1. 幼兒僅在出生時打過一兩次預防針，後續常規疫苗皆未按時接種且家長無法交代合理原因。\n2. 幼兒的身高與體重對照生長曲線表，落於小於第 3 百分位的位置。\n3. 幼兒在預防接種打針時，會大聲哭鬧及用力掙扎。\n4. 脫下衣物準備打針時，發現幼兒身上有符合「甜甜圈的洞」或「斑馬紋」特徵的燙傷痕跡。",
    "1、2、3",
    "1、2、4",
    "2、3、4",
    "1、3、4",
    "",
    "B",
    "敘述 3： 錯誤。幼兒在打針時因疼痛或害怕而大聲哭鬧及掙扎，是所有兒童皆會有的正常生理與情緒反應，並非兒少受虐或疏忽的指標。敘述 1 為醫療疏忽，敘述 2 為營養疏忽或生長遲滯，敘述 4 則是典型的虐待性浸泡燙傷特徵。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "小華兩歲了，由祖母帶來預防接種門診補打多劑過期的疫苗。醫師查閱病歷與手冊發現，小華不僅預防接種紀錄極不完整，且過去曾有嚴重尿布疹與皮膚病未就醫的紀錄。這最可能是哪一種兒少保護案件的表徵？",
    "身體虐待",
    "精神虐待",
    "疏忽",
    "代理孟喬森症候群",
    "",
    "C",
    "A、B、D： 錯誤。孩子應就醫而未就醫，例如未定期預防接種、嚴重皮膚病未處理，未給予適當醫療需求，屬於典型的疏忽表徵中的醫療面疏忽。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "醫事人員在預防接種看診時，除了評估嬰幼兒的生理狀況外，也應觀察家長與幼兒的互動。下列哪些家長或照顧者的特徵與行為，屬於兒虐或疏忽的危險因子，需醫事人員多加留意？\n1. 對於嬰幼兒的發展遲緩或體重過輕，表現出漠不關心或有缺乏醫學根據的特殊想法。\n2. 照顧者本身有明顯精神疾病（如重度憂鬱症）或有濫用酒精、藥物的經驗。\n3. 在診間對幼兒的要求極端不合情理（不符合其發展能力），或時常以言語恐嚇、貶損幼兒。\n4. 為了安撫幼兒打針後的情緒，給予貼紙或擁抱做為獎勵。",
    "1、2、3",
    "1、3、4",
    "2、3、4",
    "1、2、4",
    "",
    "A",
    "敘述 4： 錯誤。給予貼紙或擁抱安撫打針後的幼兒，是合適且正向的教養與安撫行為。敘述 1、2、3 則分別是疏忽、高風險家庭以及精神虐待的常見危險因子與表徵。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "護理師在預防接種門診時，發現一名 1 歲幼兒除了疫苗嚴重延遲外，軀幹與耳朵有多處新舊雜陳的瘀傷。家長解釋是「小孩學走路常跌倒」。依據 TEN-4 BCDR 法則及通報規定，醫事人員的正確處置應為何？",
    "軀幹與耳朵是學步期幼兒跌倒最常受傷的部位，家長解釋合理，不需特別處理。",
    "應立刻與家長爭辯，並指責家長說謊，隨後由醫院逕行扣留幼兒。",
    "醫事人員無須扮演偵探查證事實真相，只要有合理懷疑，即應於 24 小時內進行兒少保護通報。",
    "為了維持醫病關係，應先要求家長保證下次會按時打疫苗，若再違規才進行通報。",
    "",
    "C",
    "A： 錯誤。依據 TEN-4 法則，小於 4 歲兒童的軀幹、耳朵、頸部瘀傷應高度懷疑身體虐待，並非常見的意外跌倒部位。\nB： 錯誤。醫事人員應保持中立客觀，避免激怒疑似施虐者導致醫病關係破裂或造成兒少立即危險。\nD： 錯誤。通報是法定義務，不能因為家長保證或維持醫病關係而免除或延遲通報。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "關於評估兒少是否有醫療方面疏忽，預防接種門診是絕佳的把關關卡。當醫事人員在門診發現下列哪些情形時，應警覺可能為疏忽個案？\n1. 幼兒患有氣喘等慢性疾病，但家長未遵循醫囑給予藥物控制，僅在急性發作時送急診。\n2. 嬰幼兒的健康手冊上，預防接種紀錄多處空白，且家長對此漠不關心。\n3. 幼兒有嚴重的尿布疹或皮膚病，但家長卻從未帶其就醫處理。\n4. 家長因工作繁忙，委託具有合格執照的保母帶幼兒來衛生所施打常規疫苗。",
    "1、2、3",
    "1、3、4",
    "2、3、4",
    "1、2、4",
    "",
    "A",
    "敘述 4： 錯誤。委託合格保母代為照顧並帶領幼兒完成應有的預防接種，是適當且負責任的照顧行為，不屬於疏忽。敘述 1、2、3 皆為兒少保護醫療面疏忽的明確指標。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "醫師在預防接種前進行理學檢查時，發現病童手臂有不明瘀傷，欲進一步脫下病童衣物檢查全身是否有其他新舊傷痕。但媽媽強烈拒絕，要求醫師「只要打針就好」，並表示自己有權利拒絕醫師的檢查。此時醫師應如何處置？",
    "尊重媽媽的決定，只針對露出的部位進行記錄後直接施打疫苗，不需通報。",
    "在診斷書或病歷上據實記載「病童的媽媽拒絕檢查全身」，並依法進行通報。",
    "強制將媽媽趕出診間，並請警衛壓制病童強制脫衣檢查。",
    "拒絕為病童施打疫苗，並請媽媽帶去其他診所。",
    "",
    "B",
    "A、D： 錯誤。當發現可疑傷痕且家長拒絕檢查，仍需依法通報兒少保護事件，不應放棄通報或拒絕醫療。\nC： 錯誤。不宜使用強暴脅迫手段破壞醫病關係，應據實記載家長拒絕檢查之事實，後續交由社政與警政人員調查。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "在預防接種門診中，醫師明確告知病童家長因發現疑似受虐傷痕，依法必須進行通報。家長當場大怒，威脅醫師：「如果你敢通報洩漏我的個資，我就去告你違反個人資料保護法！」關於此情境，醫師的正確觀念為何？",
    "家長有權拒絕個人資料被提供給社政單位，醫師應停止通報以避免被告。",
    "醫師依法通報兒少保護事件屬履行法定義務，得免告知當事人，不會違反個人資料保護法。",
    "醫師應先請家長簽署「同意通報切結書」後，才能進行通報程序。",
    "醫師應將病歷交給家長帶走，以平息家長的憤怒。",
    "",
    "B",
    "A、C、D： 錯誤。兒少保護事件之通報為法定義務。依個人資料保護法規定，履行法定義務所必要者得免告知當事人，且不需取得家長同意。若醫事人員受威脅，還可依法請求警察機關協助。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "一名 5 歲女童來衛生所接種常規疫苗。排隊等待時，女童因害怕而小聲啜泣。父親不顧旁人，當眾大聲辱罵女童：「妳這個沒用的賠錢貨，再哭我就把妳丟到垃圾桶給狗咬死！」且用力將女童推開。這種持續且不合常理的恐嚇與貶損行為，應被歸類為哪一種兒少虐待？",
    "身體虐待",
    "精神虐待",
    "疏忽",
    "代理孟喬森症候群",
    "",
    "B",
    "A、C、D： 錯誤。使用語言和肢體動作排斥、貶損兒少，或威脅恐嚇拋棄兒少，導致兒少情緒行為遭受不良影響，屬於精神虐待的典型類型與臨床表徵。",
    60,
    "timeBucket",
    false,
    false,
  ...
  [
    ...
    "single",
    "兒少虐待與疏忽測驗題（預防接種實務專篇）",
    "護理人員在協助 4 歲男童脫褲子準備在大腿施打常規疫苗時，男童表現出極度的驚恐、強烈抗拒脫褲子，甚至突然出現與年齡不相符的性暗示言語及動作。同時，護理人員發現其生殖器周圍有不明原因的紅腫。醫事人員應高度懷疑男童可能遭遇何種事件？",
    "疫苗過敏反應",
    "嚴重的精神分裂症",
    "兒少性虐待",
    "生長遲滯",
    "",
    "C",
    "A、B、D： 錯誤。兒少抗拒如廁或脫衣、對性有與年齡不相符的不尋常知識與言行、突然出現性暗示語言，以及生殖器或肛陰部不明原因紅腫疼痛，皆是兒少性虐待的臨床行為與身體指標，醫事人員須立刻提高警覺。",
    60,
    "timeBucket",
    false,
    false,
...
function closeQuestionAndRevealAnswer(data) {
  ...
  const nextState = withLatestFirebaseTreasureState(gameId, {
    gameId,
    status: 'question_closed',
    currentQuestionId: questionId,
    questionOpenedAt: '',
...
function scoreClosedQuestionNow(data) {
  ...
  if (currentState.status === 'question_closed' && currentState.currentQuestionId === questionId) {
    firebaseSync = publishGameStateToFirebase(withLatestFirebaseTreasureState(gameId, {
      gameId,
      status: 'question_closed',
      currentQuestionId: questionId,
      questionOpenedAt: '',
