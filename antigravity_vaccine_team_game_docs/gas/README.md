...
18. 第 3 版 `0.3.20` 起，學員端報到、送答、道具、寶箱、成就、創作投稿與投票高頻操作開始改寫 Firebase 暫存資料；一般題送答遇 Firebase 寫入失敗會回退 GAS；創作投稿與投票以 `questionId` 隔離目前創作題；GAS 保留賽前同步、管理初始化、賽後匯出與正式重算方向。
19. 第 7 版 `0.7.5` 起，提供管理 action `getSettlementBatchStatus` 查詢 Firebase `settlementBatches/{gameId}` 批次狀態摘要，供關題後監看背景計分是否完成。
20. 活動結束後從 Google Sheets 匯出：
   - 作答紀錄
   - 戰隊成績
   - 個人成績
   - 得獎名單
