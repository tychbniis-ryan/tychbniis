...
18. 第 3 版 `0.3.17` 起，學員端報到、送答、道具、寶箱、成就、創作投稿與投票高頻操作開始改寫 Firebase 暫存資料；GAS 關題與結算會發布 `publicScoreboards` 快照；GAS 保留賽前同步、管理初始化、賽後匯出與正式重算方向。
...
第 3 版 0.3.17 起，學員報到、送答、道具使用、成就領取、寶箱開啟、創作投稿與投票的現場高頻操作開始改寫 Firebase 暫存節點。`closeAndScoreQuestion` 會發布暫時排行榜快照到 `publicScoreboards/{gameId}`，`finalizeCompetition` 會發布正式排行榜快照。既有 GAS action 先保留為備援與正式結算基礎，後續需新增從 Firebase 匯出 `players`、`answers`、`itemUses`、`treasureBoxOpenRequests`、`achievementClaimRequests`、`creativeSubmissions`、`creativeTeamVotes`、`creativeFinalVotes` 並以 Google Sheets answerKey 重新計分的流程。
